...
meta = meta.loc[:, ~meta.columns.str.contains('nside')]
...
for dataset in datasets:
    ...
    output.loc[dataset] = f.fdrcorrection(output.loc[dataset])[1]
power = -output.apply(np.log10)

f.setupplot()
f.clustermap(power.T, figsize=(4.5,4))
f.savefig('power')
